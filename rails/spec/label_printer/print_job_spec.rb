...
RSpec.describe LabelPrinter::PrintJob, type: :model do 
  ...
  it "should have some values" do
    expect(build(:print_job, label_template_id: label_template.id, data: label_template.dummy_values.to_h)).to be_valid
    expect(build(:print_job, data: nil)).to_not be_valid
    expect(build(:print_job, data: {})).to_not be_valid
  end

  it "should print the label if it is valid" do
    print_job = LabelPrinter::PrintJob.build(printer_name: printer.name, label_template_id: label_template.id, data: label_template.dummy_values.to_h)
  ...
  it "should not print the label if it is not valid" do
    print_job = LabelPrinter::PrintJob.build(printer_name: printer.name, label_template_id: 999, data: label_template.dummy_values.to_h)
  ...
  it "with valid data should carry out the print job" do
    print_job = LabelPrinter::PrintJob::LPD.new(printer_name: printer.name, label_template_id: label_template.id, data: label_template.dummy_values.to_h)
    allow(print_job).to receive(:system)
    expect(print_job.execute).to be_truthy
  end

  it "with invalid data should not carry out the print job" do
    print_job = LabelPrinter::PrintJob::LPD.new(printer_name: printer.name, label_template_id: label_template.id, data: {})
  ...
  it "with a particular protocol should build a print job of the correct class" do
    ...
    print_job = LabelPrinter::PrintJob.build(printer_name: create(:printer, protocol: "IPP").name, label_template_id: label_template.id, data: label_template.dummy_values.to_h)
