...
RSpec.describe V1::LabelTypesController, type: :request do |variable|
  ...
  it "should allow creation of a new label type" do
    expect {
      post v1_label_types_path, {data:{type: :label_type, attributes:attributes_for(:label_type)}}.to_json, 'ACCEPT' => "application/vnd.api+json", 'CONTENT_TYPE' => "application/vnd.api+json" 
  ...
  it "should prevent creation of a new label type with invalid attributes" do
     expect {
      post v1_label_types_path, {data:{type: :label_type, attributes:attributes_for(:label_type).except(:name)}}.to_json, 'ACCEPT' => "application/vnd.api+json", 'CONTENT_TYPE' => "application/vnd.api+json"
  ...
  it "should allow update of existing label type" do
    ...
    patch v1_label_type_path(label_type), {data:{type: :label_type, attributes:{ name: new_name }}}.to_json, 'ACCEPT' => "application/vnd.api+json", 'CONTENT_TYPE' => "application/vnd.api+json"
    expect(response).to be_success
  ...
  it "should prevent update of existing label type with invalid attributes" do
    label_type = create(:label_type)
    patch v1_label_type_path(label_type), {data:{type: :label_type, attributes:{ name: nil }}}.to_json, 'ACCEPT' => "application/vnd.api+json", 'CONTENT_TYPE' => "application/vnd.api+json"
    expect(response).to have_http_status(:unprocessable_entity)
    expect(ActiveSupport::JSON.decode(response.body)["errors"]).not_to be_empty
  end
