...
RSpec.describe V1::PrintJobsController, type: :request do
  ...
  it "should print a valid label" do
    ...
    post v1_print_jobs_path, {data: {type: 'print_job', attributes: { printer_name: printer.name, label_template_id: label_template.id, labels: label_template.dummy_labels.to_h}}}.to_json, {'ACCEPT' => "application/vnd.api+json", 'CONTENT_TYPE' => "application/vnd.api+json"} 
  ...
  it "should return an error if the label is not valid" do
    post v1_print_jobs_path, {data: {type: 'print_job', attributes: { label_template_id: label_template.id, labels: label_template.dummy_labels.to_h}}}.to_json, {'ACCEPT' => "application/vnd.api+json", 'CONTENT_TYPE' => "application/vnd.api+json"}
