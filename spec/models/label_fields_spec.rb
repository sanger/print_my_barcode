...
RSpec.describe LabelFields, type: :model do
  ...
  let(:labels) do
    [create(:label, name: 'header', bitmaps: [create(:bitmap, field_name: 'header_text_1'), create(:bitmap, field_name: 'header_text_2')]),
  ...
  it 'creates header variables' do
    expect(label_fields.find(:header)).to eq(%w[header_text_1 header_text_2])
  end

  it 'creates labels variables' do
    expect(label_fields.find(:label1)).to eq(%w[label_text_1 label_text_2])
    expect(label_fields.find(:label2)).to eq(%w[label_text_3 label_text_4])
  end

  it 'creates footer variables' do
  ...
  it 'provides an actual count which is based on the actual number of dummy values' do
