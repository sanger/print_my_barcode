...
RSpec.describe Squix::PrintJob do
  ...
  describe 'new' do
    it 'has the correct instance variables' do
      print_job = described_class.new(attributes)
    ...
    it 'does not be valid without the printer name' do
      print_job = described_class.new(attributes.except(:printer_name))
      expect(print_job).not_to be_valid
    end

    it 'does not be valid without the label_template_name' do
      print_job = described_class.new(attributes.except(:label_template_name))
      expect(print_job).not_to be_valid
    end

    it 'does not be valid without the labels' do
      print_job = described_class.new(attributes.except(:labels))
      expect(print_job).not_to be_valid
    end
  end
  ...
  describe '#execute' do
    context 'success' do
      it 'sends a print request and returns true' do
        print_job = described_class.new(attributes)
        allow(SPrintClient).to receive(:send_print_request)
          .and_return Net::HTTPResponse.new('1.1', '200', '')
        expect(SPrintClient).to receive(:send_print_request)
          .with(print_job.printer_name, "#{print_job.label_template_name}.yml.erb", print_job.merge_fields_list)
    ...
    context 'failure' do
      it 'sends a print request and returns false' do
        print_job = described_class.new(attributes)
        allow(SPrintClient).to receive(:send_print_request)
          .and_return Net::HTTPResponse.new('1.1', '422', 'an error')
        expect(SPrintClient).to receive(:send_print_request)
          .with(print_job.printer_name, "#{print_job.label_template_name}.yml.erb", print_job.merge_fields_list)
  ...
  describe '#merge_fields_list' do
    it 'returns the correct number of merge fields when copies is more than 1' do
      print_job = described_class.new(attributes.merge(copies: 2))
