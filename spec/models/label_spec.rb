...
RSpec.describe Label, type: :model do
  ...
  it 'label should not be valid without a name' do
    expect(build(:label, name: nil)).not_to be_valid
  end
  ...
  it 'label should be unique for a particular label template' do
    ...
    expect(build(:label, label_template: label_template_1, name: label.name)).not_to be_valid
  ...
  it 'name should only be valid in a particular format' do
    ...
    expect(build(:label, name: 'label 1')).not_to be_valid
    expect(build(:label, name: 'label-1')).not_to be_valid
    expect(build(:label, name: 'label1*')).not_to be_valid
  end
  ...
  it 'dups a label correctly' do
    ...
    label.drawings.pluck(:id).each do |id|
      expect(duped_label_drawing_ids).not_to include(id)
    end
  end
end
