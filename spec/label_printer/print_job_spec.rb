...
RSpec.describe LabelPrinter::PrintJob, type: :model do
  ...
  it 'has some values' do
    ...
    expect(build(:print_job, labels: nil)).not_to be_valid
    expect(build(:print_job, labels: {})).not_to be_valid
  end

  it 'prints the label if it is valid' do
  ...
  it 'does not print the label if it is not valid' do
