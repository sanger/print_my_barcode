module LabelPrinter
  module Commands
    ...
    class AdjustPosition < Commands::Base
      # Description: Adjusts the feed length, cut position and
      # back feed length.
      # Format: [ESC] AX; abbb, cddd, eff [LF] [NUL]
      # Example: AX;+004,+000,+00
      # a: direction in which fine adjustment to be made 
      # forward(+) or backward(-) normally backward.
      # bbb: Feed value to be finely adjusted (000 - 500)
      # c: direction forward (-) or backward (+) in which a cut 
      # position fine adjustment is to be made.
      # ddd: Amount for finely adjusting the cut position (000 - 180)
      # e: Indicates whether the back feed is to be 
