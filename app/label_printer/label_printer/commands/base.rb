module LabelPrinter
  module Commands
    ##
    # A command is something which when sent to the
    # printer will perform a certain action
    ...
    class Base
      ...
      ##
      # The control codes are the ordered set of instructions
      # which make up a valid command without the prefix or
      # escape characters.
      def control_codes
        'CC12345'
      end
      ...
      ##
      # A formatted command is one which has the prefix, separator and
      # control codes e.g. PF;ABCDE12345
      def formatted(separator = '')
        "#{prefix}#{separator}#{control_codes}"
      end

      ##
      # The output which is sent to the printer includes formatted
      # command with escape characters e.g. [ESC]PF;ABCDE12345[LF][NUL]
      def to_s
      ...
      # This method is useless unless all other commands can
